...
/// что пользователь просит не «поговорить», а выполнить сценарий. Ловля двухступенчатая:
/// сначала системный гейт (без сети), затем HIT_PROMPT в дешёвую инстант-модель, от которой
/// ждём «ДА». Действия с прошедшим гейтом пробуются по очереди до первого попадания;
/// не попало ни одно — сообщение уходит дальше по пайпу, в обычную сессию.
/// Проверка попадания ждётся под канальным замком MessageHandler: от её исхода зависит,
/// потреблено сообщение или нет. Держать замок секунду допустимо потому, что до неё
/// доходят единицы сообщений. Сам процессор работает минутами и уходит в фон.
/// </summary>
