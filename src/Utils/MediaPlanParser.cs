...
public static class MediaPlanParser
{
    ...
    /// <summary>
    /// Разбирает ответ модели в план. null — это не JSON: просьбу не поняли.
    /// Числа берутся мягко (модель может прислать строку), и кроме них в аргументы ffmpeg
    /// отсюда попадает только формат, который дальше проверяется по белому списку.
    /// </summary>
    ...
    /// Нужен медиа-сессии — накопленный план уезжает в БД и обратно в промпт.
    /// Пишется штатным писателем: он экранирует, поэтому ни табуляции, ни перевода
    /// строки в результате не будет, а на этом держится формат файла сессий.
    /// </summary>
