...
/// <summary>
/// Эмодзи приложения — единственный способ показать свою картинку прямо в тексте сообщения:
/// серверные эмодзи живут в одной гильдии, а эти Discord хранит у приложения и отдаёт боту
/// везде. Заводятся один раз за всю жизнь бота, поэтому при старте достаточно проверить,
/// что нужные уже есть.
/// </summary>
public static class BotEmotes
{
    ...
    /// <summary>
    /// Логотип Telegram. null означает, что эмодзи не завелось (не хватило прав, не нашлась
    /// картинка): подписи просто останутся без иконки, остальное работает.
    /// </summary>
    public static Emote? Telegram { get; private set; }

    /// <summary>
    /// Логотип X — на том же положении, что и <see cref="Telegram"/>.
    /// </summary>
